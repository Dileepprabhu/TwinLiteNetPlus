...
from types import SimpleNamespace  # Import to create a temporary args object
...
def select_device(device='', batch_size=0, newline=True):
    ...
    if cpu or mps:
        ...
        if not torch.cuda.is_available():
            ...
            assert torch.cuda.device_count() >= len(device.replace(',', '')), \
...
def run(weights='', imgsz=[], batch_size=1, device='', include=[], output='', config='small'):
    # Create a temporary args object with the config attribute
    args = SimpleNamespace(config=config)
    model = Net(args=args)  # Pass the args object to Net
    model = model.to(select_device(device))
    ...
    file = Path(output) if output else Path(weights)  # use output if provided, else weights
    # Input
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str,
                        default='pretrained/small.pth', help='model.pt path(s)')
